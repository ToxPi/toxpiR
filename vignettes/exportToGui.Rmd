...
To export a toxpiR model object to the GUI format D input you can follow the example below and modify to fit your own data. We recommend keeping analyses within the R package environment once started, but this is a way for backwards compatibility.

Note on slice weights: Currently the Java GUI only allows weights that are either an integer or a ratio of integers. If your weights are irrational, the best approximation would be to find an integer ratio with the closest value.
...
# NOTE: different transformation functions within a slice are not supported, e.g.
...
# NOTE: "NULL" is set as "linear(x)" below
