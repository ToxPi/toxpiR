...
#### TxpSlice(txpValueNames, txpTransFuncs, txpLowerNames, txpLowerFuncs, txpUpperNames, txpUpperFuncs)
...
> *txpLowerNames* : character vector indicating which data to include in the lower CI of the slice. Can just be the name of a single column if that is all the slice contains.

> *txpLowerFuncs* : List of transformation functions to use with one function per entry in txpLowerNames. Can be created using TxpTransFuncList() method. Defaults to NULL. 

> *txpUpperNames* : character vector indicating which data to include in the upper CI of the slice. Can just be the name of a single column if that is all the slice contains.

> *txpUpperFuncs* : List of transformation functions to use with one function per entry in txpUpperNames. Can be created using TxpTransFuncList() method. Defaults to NULL. 
...
## Goal - Create three slices with transformation functions and increasing complexity
...
f.slices <- TxpSliceList(Slice1 = TxpSlice(txpValueNames = "Slice1_M1"), 
                         ...
                         Slice3 = TxpSlice(txpValueNames = c("Slice3_M1", "Slice3_M2"),
                                           txpLowerNames = c("Slice3_L1", "Slice3_L2"),
                                           txpUpperNames = c("Slice3_U1", "Slice3_U2"),
                                           txpUpperFuncs = slice3.upper.trans))
...
txpExportCSV(fileName = "txp_vignette_model.csv", model = f.model, input = txp_example_input_CI)
```
...
```{r}
f.model <- txp_example_model_CI #Load a more complex model than we created previously

#Calculate results, integer 1 is provided for id.var specifying the first column is the id column in the input
...
txpSliceLows(f.results) #ToxPi lower CI slice scores
txpSliceUps(f.results) #ToxPi upper CI slice scores
...
# Add more missing data to slice 2 via metrics named Slice2_M1 and Slice2_M2 in the raw data
f.input[2:8, "Slice2_M1"] <- NA
f.input[3:10, "Slice2_M2"] <- NA

# Modify metric transformation functions for slice 3
txpTransFuncs(txpSlices(f.model)[[3]]) <- TxpTransFuncList(f1 = \(x) sqrt(x - 1.5), f2 = \(x) x)
...
#change first slice name. Note: changing models/results is not recommended for actual results objects. Please create a new model with real data
names(f.results@txpModel)[1] <- "long slice name"
colnames(f.results@txpSliceScores)[1] <- "long slice name"
colnames(f.results@txpSliceLows)[1] <- "long slice name_low"
colnames(f.results@txpSliceUps)[1] <- "long slice name_up"
...
colnames(f.results@txpSliceLows)[1] <- "ExampleA_low"
colnames(f.results@txpSliceUps)[1] <- "ExampleA_up"
...
plot(f.results, y = txpRankLows(f.results), labels = 1:10)
plot(f.results, y = txpRankUps(f.results), labels = 1:10)
```
```{r fig.width=5, fig.height=4, fig.align='center'}
# Hierarchical Clustering
f.hc <- hclust(dist(txpSliceScores(f.results)))
f.hc.low <- hclust(dist(txpSliceLows(f.results)))
f.hc.up <- hclust(dist(txpSliceUps(f.results)))
