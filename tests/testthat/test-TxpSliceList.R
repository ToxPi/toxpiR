...
test_that("We can access TxpSlice slots from TxpSliceList", {
  ...
  expect_s4_class(txpTransFuncsLower(sl, simplify = TRUE), "TxpTransFuncList")
  expect_s4_class(txpTransFuncsUpper(sl, simplify = TRUE), "TxpTransFuncList")
  expect_length(txpTransFuncs(sl, simplify = TRUE), 3)
  expect_length(txpTransFuncsLower(sl, simplify = TRUE), 0)
  expect_length(txpTransFuncsUpper(sl, simplify = TRUE), 0)
  expect_null(txpValueNamesUpper(sl))
  expect_null(txpValueNamesLower(sl))
  sl <- TxpSliceList(S1 = TxpSlice(txpValueNamesLower = "inpt1"), S2 = TxpSlice(txpValueNamesUpper = c("inpt2", "inpt3")))
